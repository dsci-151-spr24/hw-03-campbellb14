...
Answers to this question are stored in the variable in our dataset.

1.  What are the possible responses to this question and how many respondents chose each of these answers?

```{r}
responsecounts <- table(gss16$harass5)
responsetable <- as.data.frame(responsecounts)
colnames(responsetable) <- c("Response", "Count")
...
```{r}
responses <- gss16$harass5[!is.na(gss16$harass5) & 
gss16$harass5 != "Does not apply"]

total_valid <- length(responses)
harassed_count <- sum(responses == "Yes")
...
ggplot(gss16, aes(x = email)) +
  geom_histogram(binwidth = 250, fill = "red", color = "gray") +
  labs(title = "Distribution of Weekly Email Time", x = "Minutes", y = "Frequency")
```
```{r}
...
***The data is skewed to the right, because people spend so little time on their email most frequently. The median is a better measure of the typical amount of time Americans spend on email weekly because it the other outlier may make the average number not accurate.***
...
```{r}
valid_responses <- gss16$snap_insta[!is.na(gss16$snap_insta)] 

valid <- length(valid_responses)
count <- sum(valid_responses == "Yes")
...
```{r}
responsecounts2 <- table(gss16$wrkstat)
responsetable2 <- as.data.frame(responsecounts2)
colnames(responsetable2) <- c("Response", "Count")

print(responsetable2)
```
...
***The slope of the variables can be interpreted by how high or low the value is. If a value is positive, then the variable is associated with more minutes spent on email. If a value is negative, then the variable is associated with less minutes spent on email.***
...
***There are a lot of outliers in the positive residuals while majority of the cluster is actually around 0. This is an issue.***
...
***The visualization shows that people of different political views seem to have similar views on whether they support advancing technology or not. The majority of each of the political views do support advancing technology.***
